...
async def main():
    ...
    initial_state = {
        ...
        "target_keyword": "Dmart near me",
        "secondary_keywords": ["Dmart offers", "Dmart stores"],
        "messages": []
    }
    ...
    try:
        ...
        logger.info(f"Featured Image: {result.get('featured_image', 'N/A')}")
        logger.info(f"URL Slug: {result.get('url_slug', 'N/A')}")
        logger.info(f"Meta Description: {result.get('meta_description', 'N/A')}")
        logger.info(f"Tags: {result.get('tags', [])}")
        logger.info(f"Image Alt Text: {result.get('image_alt_text', 'N/A')}")
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            if result.get('url_slug'):
                f.write(f"**URL Slug:** /{result['url_slug']}\n\n")
            if result.get('tags'):
                f.write(f"**Tags:** {', '.join(result['tags'])}\n\n")

            if result.get('featured_image'):
                alt_text = result.get('image_alt_text', 'Featured Image')
                f.write(f"![{alt_text}]({result['featured_image']})\n\n")
